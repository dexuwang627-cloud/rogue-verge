...
import DataStreamBorder from '../components/UI/DataStreamBorder';

const RelicDetail = ({ lang }) => {
    const { id } = useParams();
    const item = RELICS_DATA.find(r => r.id === parseInt(id));
    ...
    if (!item) return <div className="text-white text-center pt-32">Relic not found.</div>;
    ...
    const altText = `Rogue Verge ${item.code} - ${displayNote} - 手工賽博圖騰聖物細節圖 ${currentSlide + 1}`;
        ...
        <div className="pt-32 px-6 md:px-20 min-h-screen pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                ...
                <div className="relative border border-white/10 bg-[#050505] p-4 group">
                    ...
                    {totalSlides > 1 && (
                        <React.Fragment>
                            <button onClick={handleNext} aria-label="Next image" className="absolute right-0 top-1/2 -translate-y-1/2 bg-black/50 p-3 text-white/70 hover:text-white transition-colors z-20">&gt;</button>
                            <button onClick={handlePrev} aria-label="Previous image" className="absolute left-0 top-1/2 -translate-y-1/2 bg-black/50 p-3 text-white/70 hover:text-white transition-colors z-20">&lt;</button>
                ...
                <div className="space-y-8">
                    <div>
                        <h1 className="font-serif text-4xl md:text-5xl text-white tracking-tight mb-2">{item.code}</h1>
                        <h2 className="font-serif text-sm text-red-500 tracking-widest">{displayNote}</h2>
                    </div>
