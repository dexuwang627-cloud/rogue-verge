        ...
        <div className="pt-32 px-6 md:px-20 min-h-screen pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {RELICS_DATA.map((item) => {
                    ...
                    const altText = `Rogue Verge 檔案 - ${item.code} - ${displayNote} - 賽博圖騰風格`;

                    return (
                        <Link
                            to={`/relics/${item.id}`}
                            key={item.id}
                            className="group relative aspect-square border border-white/10 bg-[#050505] overflow-hidden cursor-pointer block"
                        >
                            <div className="absolute inset-0 w-full h-full">
                                <img src={item.src} alt={altText} className="w-full h-full object-cover opacity-60 grayscale transition-all duration-700 group-hover:opacity-100 group-hover:grayscale-0 group-hover:scale-110" />
                            </div>
